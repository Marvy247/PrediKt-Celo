...
export default function CreateBattle() {
  ...
  const validateForm = () => {
    ...
    if (parseFloat(stakeAmount) <= 0) return "Stake must be greater than 0";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (chain?.id !== CELO_MAINNET_CHAIN_ID) {
      ...
      try {
        ...
        const errorMsg = `Failed to switch to Celo network: ${err instanceof Error ? err.message : 'Unknown error'}`;
        setError(errorMsg);
        toast.error(errorMsg);
        setIsSwitchingChain(false);
        return;
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="p-4 rounded-lg bg-gradient-to-r from-green-500/10 to-blue-500/10 border border-green-500/20">
                      ...
                      <p className="text-2xl font-bold text-primary">
                        {(parseFloat(stakeAmount || '0') * 2).toFixed(3)} CELO
                      </p>
